...
const AppHeader = () => {
    ...
    const handleMenuClick = (key) => {
      ...
      setOpen(false); // đóng drawer nếu đang mở (mobile)
    };
    ...
    const datKhamMenuItems = [
          {
              ...
              label: (
                <div onClick={() => {
                  ...
                  Đặt khám bác sĩ
                  <br /><span className='txt-nho'>Đặt lịch khám không chờ đợi</span>
                </div>
              ),
          },
          {
              ...
              label: (
                <div onClick={() => {
                  ...
                  Đặt khám bệnh viện
                  <br /><span className='txt-nho'>Đặt khám, thanh toán, nhận kết quả</span>
                </div>
              ),
          },
          {
              ...
              label: (
                <div onClick={() => {
                  ...
                  Đặt khám phòng khám
                  <br /><span className='txt-nho'>Đa dạng chuyên khoa và dịch vụ</span>
                </div>
              ),
          },
          {
              ...
              label: (
                <div onClick={() => {
                  ...
                  Đặt khám tiêm chủng
                  <br /><span className='txt-nho'>Trung tâm tiêm chủng uy tín</span>
                </div>
              ),
          },       
          { type: 'divider' },
      ...
      >
          ...
          <Menu.Item key="timyte" onClick={() => navigate('/search')}>Tìm Y tế</Menu.Item>
          <Menu.Item key="cauhoithuonggap" onClick={() => navigate('/cau-hoi-thuong-gap')}>Câu hỏi thường gặp</Menu.Item>
      </Menu>
      );
      ...
      const items = [
        {
          key: '1',
          label: <label style={{cursor:"pointer", fontSize: 17}} onClick={() => setOpenModalDoiMK(true)}>Tài khoản của tôi</label>,
        },   
        {
            key: '2',
            label: <label style={{cursor:"pointer", fontSize: 17}} onClick={() => handleRedirectLichHen(acc?._id)}>Lịch hẹn</label>,
          },      
        {
          key: '4',
          danger: true,
          label: <label style={{cursor:"pointer", fontSize: 17}} onClick={() => handleLogout()}>Đăng xuất</label>,
        },
      ];
                ...
                <>
                    ...
                    >
                      <div style={{cursor:'pointer'}}>
                        <Avatar src={`${import.meta.env.VITE_BACKEND_URL}/uploads/${acc.image}`} style={{cursor: "pointer"}} size={35} icon={<UserOutlined />} /> &nbsp;
                        <span style={{fontSize: 18}}>{acc.lastName} {acc.firstName}</span> <DownOutlined />
                      </div>
                    </Dropdown>
                </> : 
                <>
                  <Button className="login-button" size="large" onClick={() => setOpenModalLogin(true)}>
                    Đăng nhập
                  </Button>
                </>}  
